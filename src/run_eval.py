...
Usage:
    ...
    python src/run_eval.py --model gpt --workers 4

    # Remote env host — agents connect to pre-running servers
    python src/run_eval.py --model gpt --workers 8 \\
        --env-host ec2-host --base-port 8001

    # Filter tasks
    python src/run_eval.py --model claude --difficulty easy --workers 2

    # Multi-run: repeat full task set 3 times
    python src/run_eval.py --model gemini-pro --repetitions 3 --workers 4

    # Multi-run cascading: run all, retry failures, retry remaining failures
    python src/run_eval.py --model gemini-pro --repetitions 3 --failed-only --workers 4
"""
...
# Ensure sibling modules are importable when run as `python src/run_eval.py`
...
async def main():
    ...
    parser.add_argument("--base-port", type=int, default=13001, help="First port for env instances")
    parser.add_argument(
        "--env-host",
        default="localhost",
    ...
    if args.repetitions <= 1:
        if args.resume_dir:
            ...
            # For local-served models the actual model identity comes from
            # env vars (QWEN_MODEL etc), not --model. Append a slug of that
            # so 9B-sft and 27B runs don't collapse into the same name.
            served = os.environ.get("QWEN_MODEL") if args.model == "qwen" else ""
            served_slug = ""
            if served:
                s = served.lower()
                for ch in (".", "/", " "):
                    s = s.replace(ch, "-")
                served_slug = "_" + s.strip("-")[:40]
            run_dir = Path(output_dir) / (
                f"{args.model}{served_slug}_{timestamp}{suite_tag}{extra_tag}_parallel"
            )
