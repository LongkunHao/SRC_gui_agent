...
class VisionAgentBase:
    ...
    def __init__(
        ...
        *,
        ...
        self._browser_close_timeout = float(
            os.environ.get("VISION_BROWSER_CLOSE_TIMEOUT", "10")
        )
        self._browser_launch_timeout = float(
            os.environ.get("VISION_BROWSER_LAUNCH_TIMEOUT", "45")
        )
        self._page_goto_timeout_ms = int(
            os.environ.get("VISION_PAGE_GOTO_TIMEOUT_MS", "30000")
        )
    ...
    async def _launch_browser(self) -> None:
        ...
        self._page.set_default_timeout(self._page_goto_timeout_ms)
        self._page.set_default_navigation_timeout(self._page_goto_timeout_ms)
    ...
    async def setup(self, server_url: str) -> None:
        ...
        try:
            await asyncio.wait_for(
                ...
                timeout=self._browser_launch_timeout,
            )
            await self._page.goto(
                server_url,
                wait_until="load",
                timeout=self._page_goto_timeout_ms,
            )
        except Exception:
    ...
    @classmethod
    def _normalize_key_token(cls, token: object) -> str:
        key = str(token).strip()
        if not key:
            return ""
        lower = key.lower()
        if re.fullmatch(r"f\d{1,2}", lower):
            return lower.upper()
        return cls._KEY_ALIASES.get(lower, key)

    @classmethod
    def _normalize_key_combo(cls, keys: object) -> str:
        if isinstance(keys, (list, tuple)):
            parts = list(keys)
        else:
            parts = str(keys).split("+")
        mapped = [cls._normalize_key_token(p) for p in parts]
    ...
    async def execute_action(self, action: dict) -> None:
        ...
        if kind == "click":
            ...
            keys = self._normalize_key_combo(action.get("keys", ""))
            if not keys:
                return
    ...
    async def run(self, task: str, server_url: str, task_dir: Path) -> AgentResult:
        ...
        # Per-task LLM call tracer (student + teacher both log here).
        # Subclasses (e.g. RolloutAgent) propagate this to the teacher.
    ...
    "function": {
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "pixels": {"type": "number", "description": "Signed scroll amount in pixels, range [-1000, 1000]. Negative scrolls down, positive scrolls up. Use multiples of 100; typical values are +/-300, +/-500, +/-800."},
...
class Qwen35VLAgent(VisionAgentBase):
    ...
    MODEL = os.environ.get("QWEN_MODEL", "qwen-3.5-vl")
    BASE_URL = os.environ.get("QWEN_BASE_URL", "https://api.qwen.com/v1")
        ...
        if action == "left_click":
            ...
            if isinstance(raw_keys, str):
                try:
                    ...
                    raw_keys = [raw_keys]
            keys = self._normalize_key_combo(raw_keys)
            if keys:
                actions.append({"type": "key", "keys": keys})
            ...
            try:
                ...
                pass
            # Qwen/teacher sometimes emits small integers like -3 even
            # though the schema names this field "pixels". Interpret
            # small magnitudes as 100px scroll units, while preserving
            # proper pixel values such as +/-300 or +/-800.
            if 0 < abs(pixels) <= 10:
                pixels *= 100
    ...
    def _prepare_step(self, step: int, screenshot: bytes) -> None:
        ...
        # Folding: keep exactly the last ``image_max`` un-collapsed
        # observations (incl. the current step). The previous chunk-fold
        # rule (``while > image_max: folded_prefix_k += fold_size``) could
        # advance ``folded_prefix_k`` past ``total`` and collapse the
        # CURRENT screenshot, so the model received zero images from step
        # ``image_max`` onwards and would hallucinate the screen state.
        # Use a monotonically non-decreasing fold to preserve "old steps
        # stay collapsed once collapsed" while never collapsing recent
        # observations.
        target = max(0, total - self._image_max)
        if target > self._folded_prefix_k:
            self._folded_prefix_k = target
